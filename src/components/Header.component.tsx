      ...
      <div className="flex mt-1 md:mt-0 rounded-full md:rounded-none top-0 w-4/5 md:w-full fixed md:sticky py-2 md:py-5 flex-row md:gap-y-4 justify-center px-8 md:flex-col md:h-screen text-white bg-slate-900">
        {sections.map((section, index) => (
          <div
            ...
            className={`hover:${values.selected === index ? "" : "underline"} hover:stroke-red-500 flex gap-y-1 flex-col m-2 md:flex-row w-full text-center md:text-start md:justify-center`}
            key={index}
          >
            <div
              style={{
                color: values.selected === index ? "#ef4444" : "#ffffff",
                alignSelf: "center",
              }}
            >
              {section.icon}
            ...
            <p
              className={`w-full cursor-pointer md:px-5 text-sm md:text-lg hidden md:block`}
              style={{
                color: values.selected === index ? "#ef4444" : "#ffffff",
              }}
            >
